...
export default function Header({ onCartOpen }: HeaderProps) {
  ...
  return (
    ...
    >
      ...
      {/* Mobile menu panel */}
      {mobileOpen && (
        <div className="border-t backdrop-blur-xl md:hidden" style={{ background: "var(--bg)", borderColor: "var(--border)" }}>
          <div className="space-y-1 px-4 py-4">
            ...
            <button
              onClick={toggleTheme}
              className="block w-full text-left rounded-lg px-4 py-3 text-sm font-medium transition-colors"
              style={{ color: "var(--fg-muted)" }}
              onMouseEnter={(e) => { e.currentTarget.style.background = "var(--surface)"; e.currentTarget.style.color = "var(--fg)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "var(--fg-muted)"; }}
            >
